...
benchmarks cannot be implemented for Numba. For example $np.insert()$ is not
implemented, but this function is needed for the $LN$
benchmark. Therefore the $LNS$ benchmark cannot be implemented with
Numba. Still, Numba is significant enough to be benchmarked and its
current performance are an indication of what to expect in years to come.
