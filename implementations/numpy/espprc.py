...
class Label:
    ...
    def __repr__(self):
        seq = np.array([self.at])
        tmp = self
        while tmp.pred != tmp:
            tmp = tmp.pred
            seq = np.append(seq, tmp.at)
        seq = np.flip(seq)
...
class ESPPRC:
    ...
    def solve(self):
        ...
        Q.append(0)
        # we need an array of variable-size arrays, numpy can't handle that
        labels = [ [] for x in vertices ]
        Label.resources = np.array([ x for x in range(self.nresources) ])
