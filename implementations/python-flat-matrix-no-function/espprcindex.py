...
class LabelWithIndex(espprc.Label):
    ...
    def extend(self, n, rc, d, predindex, vertex):
        nl = LabelWithIndex()
        nl.at = vertex
        nl.pred = predindex
        nl.visited = self.visited.union((vertex,))
        nl.cost = self.cost + rc[self.at*n+vertex]
        nl.length = self.length + d[self.at*n+vertex]
...
class LabelCollection:
    ...
    def extend(self, n, rc, d, fro, to):
        self.labels.append(self.labels[fro].extend(n, rc, d, fro, to))
        return len(self.labels) - 1
        ...
        while len(Qset) > 0:
            ...
            for lindex in labels[n]:
                ...
                for succ in vertices:
                    ...
                    if label.length + self.d[n*self.n+succ] + \
                       self.d[succ*self.n] > self.maxlen:
                        continue
                    ...
                    nl = lc.extend(self.n, self.rc, self.d, lindex, succ)
