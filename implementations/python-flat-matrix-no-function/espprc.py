...
class Label:
    ...
    def extend(self, nnodes, vertex, rc, d):
        nl = Label()
        nl.at = vertex
        nl.pred = self
        nl.visited = self.visited.union((vertex,))
        nl.cost = self.cost + rc[self.at*nnodes+vertex]
        nl.length = self.length + d[self.at*nnodes+vertex]
        ...
        while len(Qset) > 0:
            ...
            for label in labels[n]:
                ...
                for succ in vertices:
                    ...
                    nl = label.extend(self.n, succ, self.rc, self.d)
