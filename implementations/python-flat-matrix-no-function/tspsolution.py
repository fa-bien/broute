...
class TSPSolution:
    ...
    def first2eimprovement(self):
        ...
        for p1 in range(len(tour) - 3):
            for p2 in range(p1+2, len(tour) - 1):
                if d[tour[p1]*n+tour[p1+1]] + d[tour[p2]*n+tour[p2+1]] > \
                   d[tour[p1]*n+tour[p2]] + d[tour[p1+1]*n+tour[p2+1]]:
        ...
        for i in range(1, len(tour) - 1):
            for l in range(1, 1 + min(3, len(tour)-1-i)):
                for p in chain(range(i-1), range(i+l, len(tour)-1)):
                    delta = d[tour[i-1]*n+tour[i+l]] + d[tour[p]*n+tour[i]]\
                        + d[tour[i+l-1]*n+tour[p+1]] - d[tour[p]*n+tour[p+1]]\
                        - d[tour[i-1]*n+tour[i]] - d[tour[i+l-1]*n+tour[i+l]]
    ...
    def espprc(self, nresources=6, resourcecapacity=1, index=False):
        ...
        for i in range(n):
            for j in range(n):
                rc[i*n+j] = float(d[i*n+j] - dual[j])
        # max len: sum of best assignments
        maxlen = sum([ min([d[i*n+j] for j in range(n) if i != j])
                       for i in range(n)])
        if not index:
    ...
    def maxflow(self):
        ...
        for j in range(n):
            for i in range(n):
                cap[i*n+j] = d[i*n+j] / 1000 if d[i*n+j] > t[j] else 0.0
