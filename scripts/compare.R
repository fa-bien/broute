...
##         title <- paste0('Relative CPU effort depending on array type\n(',
##                         bench, ')')
##         p <- ggplot(subset(ds, language == lang & benchmark == bench),
...
## ds <- subset(runs, language == 'Python')
...
##         title <- paste0(inter,
##                         ': relative CPU effort depending on array type (',
##                         bench, ')')
...
## }
## for (bench in c('2-opt', 'Or-opt')) {
##     title <- paste0('Relative CPU effort of Pypy vs Numba (', bench, ')')
...
##     title <- paste0('General comparison of "fast" languages (', bench, ')')
##     p <- ggplot(subset(ds, benchmark == bench),
##                 aes(x=factor(n), y=normalised, fill=language)) +
##         geom_boxplot() + labs(x='n',
##                               y='CPU time (ratio of C++14 time)',
...
##     title <- paste0('General comparison of "interpreted" languages (',
##                     bench, ')')
##     p <- ggplot(subset(ds, benchmark == bench),
##                 aes(x=factor(n), y=normalised, fill=language)) +
##         geom_boxplot() + labs(x='n',
