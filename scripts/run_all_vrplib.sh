...
lns_langs="c++14 c++14-nested-matrix c++98 java java-nested-matrix javascript javascript-nested-matrix julia-array julia-flat-matrix julia-square-matrix pypy python rust"
...
for benchmark in "${!benchmarks[@]}"; do
	...
	(echo -e "Running \e[1m$lang\e[0m implementation of \e[1m$benchmark\e[0m benchmark"
	 (pushd $impldir'/'$lang > /dev/null
	  ...
	  popd > /dev/null
	 ) > $benchmark-$lang-vrplib-runs.csv
